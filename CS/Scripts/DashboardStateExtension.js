var DashboardStateExtension = (function () {
    ...
    // 1. Model
    var dashboardStateProperty = {
        ownerType: Model.Dashboard,
        propertyName: "DashboardState",
        defaultValue: "",
        valueType: 'string'
    };
    Model.registerCustomProperty(dashboardStateProperty);

    // 2. Viewer
    function onDashboardTitleToolbarUpdated(args) {
        var customProperties = args.dashboard.customProperties;
        var cpState = customProperties.getValue(dashboardStateProperty.propertyName);
        if (cpState) {
            args.options.actionItems.push({
                type: "button",
                icon: "iconDescription",
                tooltip: "State: " + cpState
            });
        }
    }

    function onDashboardInitialized(args) {
        var cpState = args.dashboard.customProperties.getValue(dashboardStateProperty.propertyName);

        if (cpState)
            args.component.setDashboardState(cpState);
    }

    function onDashboardStateChanged(args) {
        args.component.dashboard().customProperties.setValue(dashboardStateProperty.propertyName, args.stateString);
    }
        ...
        var popup = new DevExpress.ui.dxPopup(div, {
            ...
            contentTemplate: function (contentElement) {
                dxForm = new DevExpress.ui.dxForm(contentElement, {
                    labelLocation: "top",
                    formData: {
                        "state": dashboardControl.dashboard().customProperties.getValue(dashboardStateProperty.propertyName)
                    },
                    items: [{
                        dataField: "state",
                        label: { visible: false },
            ...
            toolbarItems: [{
                ...
                options: {
                    text: "Save",
                    onClick: function () {
                        var cpState = dxForm.option("formData")["state"];

                        dashboardControl.dashboard().customProperties.setValue(dashboardStateProperty.propertyName, cpState);
    ...
    function createMenuItem(dashboardControl) {
        var menuItem = new Designer.DashboardMenuItem(name, "State (Custom)", 9999, 68);
    ...
    function DashboardStateExtension(dashboardControl) {
        var menuItem = createMenuItem(dashboardControl);
        this.name = "DashboardState",
        this.start = function () {
    ...
    return DashboardStateExtension;
}());
